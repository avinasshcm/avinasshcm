 ...
 *  5. Deregister Device 
 */
public class JxfsMSDDemo extends JxfsBasicDemo implements IJxfsConst, IJxfsMSDConst, IJxfsIntermediateListener, IJxfsOperationCompleteListener, IJxfsStatusListener  {

	private final static String DEMO_NAME = "MSD Demo";
	private final static String DEMO_DESCRIPTION = "Executes the following commands on the Dynasty MSD Simulator:\n"+
													"\t1. Open Device\n" +
													"\t2. Write Data\n" +
													"\t3. Read Data\n" +
													"\t4. Close Device\n" + 
	...
	private final static int CMD_WRITE_DATA = 2;
	private final static int CMD_READ_DATA = 3;
	...
	public static void main( String[] args) {
		new JxfsMSDDemo( getInitString()).startDemo();
	 ...
	 * @param initString
	 */
	private JxfsMSDDemo( String initString)
	{
		super( initString);
	}
	...
	public void startDemo() {
		IJxfsBaseControl deviceControl = getDeviceControl( "CardMagneticDynastySimulator", JxfsMagStripe.class);
		if( deviceControl != null) {
        	...
        	if( deviceControl instanceof JxfsMagStripe){
        		msd_ = ( JxfsMagStripe) deviceControl;
        		runCommands();
        	}
		}
	}
	...
	private void runCommands() {
		...
		while( nextCommand_ != CMD_END) {
			try {
				switch( nextCommand_) {
					case CMD_OPEN:
						identificationID_ = msd_.open();
					break;
					case CMD_WRITE_DATA:
						// Only select first track. See Documentation on MSD devices
						Vector v = new Vector();
						v.add(new byte[]{0x30,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39});
						v.add(null);
						v.add(null);
						identificationID_ = msd_.writeData(v, false);
					break;
					case CMD_READ_DATA:
						// Select the two first tracks to be read.
						JxfsMSDTrackSelection tracksSelected = new JxfsMSDTrackSelection(true, true, false);
						identificationID_ = msd_.readData(tracksSelected);
					break;
					case CMD_CLOSE:
						identificationID_ = msd_.close();
					break;
					case CMD_DEREGISTER:
						msd_.deregisterDevice();
						nextCommand_ = CMD_END;
		    	...
		    	System.out.println( "JxfsException: " + errorCode + " = " + 
		    			DtgJxfsObjectToStringResolver.getOperationCompleteIdStringFromObject( msd_, errorCode));
