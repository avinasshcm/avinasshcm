 ...
 public class AuditNonAnalyzedWriter extends AuditSplitter
   implements Runnable
 {
   private Logger logger = null;
 
   public AuditNonAnalyzedWriter(Logger logger) {
     this.logger = logger;
   }
   ...
   private void writeFile() {
     while ((!isAnalysisCompleted.booleanValue()) || ((isAnalysisCompleted.booleanValue()) && (nonAnalyzedRecords.size() > 0))) {
       String responseFile = "";
       if (nonAnalyzedRecords.size() > 0) {
         AuditAnalyzerUtils.updateNonAnalyzedRecords("", 
           "", Boolean.valueOf(true));
         List sessionSet = new ArrayList((Collection)nonAnalyzedRecordsToBeFlushed.keySet());
         ...
         for (int i = 0; i < sessionSet.size(); i++) {
           ...
           try {
             fop.write(((String)nonAnalyzedRecordsToBeFlushed.get(sessionSet.get(i))).getBytes());
             fop.flush();
             fop.close();
           }
           catch (IOException exception) {
